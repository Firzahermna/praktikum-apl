#include <iostream>
using namespace std;

struct User {
    string username;
    string password;
};

struct Gaming {

    string nama_peralatan;
    int harga;
    int stok;
};

#define MAX 100
Gaming daftarGaming[MAX];
int totalData = 0;

void tambahData(Gaming data[], int &total) {
    if (total < MAX) {
        cout << "Masukkan Nama Peralatan Gaming : ";
            cin.ignore();
            getline(cin, data[total].nama_peralatan);
            cout << "Masukkan Harga : ";
            cin >> data[total].harga;
            cout << "Masukkan Stok : ";
            cin >> data[total].stok;

        total++;
        cout << "Berhasil menambahkan data!\n";
    } else {
        cout << "Kapasitas penuh!\n";
    }
}

void lihatData(Gaming data[], int total) {
    if (total == 0) {
        cout << "Data masih kosong\n";
    } else {
        cout << "\n----- Daftar Peralatan Gaming -----\n";
        for (int i = 0; i < total; i++) {
            cout << i + 1 << ". " << data[i].nama_peralatan
                << " | Harga: Rp" << data[i].harga
                << " | Stok: " << data[i].stok << endl;
        }
    }
}

void ubahData(Gaming data[], int total) {
    int index;
    if (total == 0) {
        cout << "Tidak ada data yang bisa diubah.\n";
    } else {
        lihatData(data, total);
        cout << "Masukkan nomor data yang ingin diubah: ";
        cin >> index;
    if (index > 0 && index <= total) {
            cout << "Masukkan Nama Baru : ";
            cin.ignore();
            getline(cin, data[index - 1].nama_peralatan);
            cout << "Masukkan Harga Baru : ";
            cin >> data[index - 1].harga;
            cout << "Masukkan Stok Baru : ";
            cin >> data[index - 1].stok;
            cout << "Data berhasil diperbarui!\n";
        } else {
            cout << "Nomor tidak ada\n";
        }
    }
}

int hapusData(Gaming data[], int total) {
int index;
    if (total == 0) {
        cout << "Tidak ada data untuk dihapus.\n";
        return total;
    }
    lihatData(data, total);
    cout << "Masukkan nomor data yang ingin dihapus: ";
    cin >> index;
    if (index > 0 && index <= total) {
        for (int i = index - 1; i < total - 1; i++) {
            data[i] = data[i + 1];
        }
        total--;
        cout << "Data berhasil dihapus!\n";
    } else {
        cout << "Nomor tidak ada\n";
    }
    return total;
}
int main() {
    
    User admin = {"Firza","090"};
    string input_nama, input_pw;
    int percobaan = 0;
    bool login_berhasil = false;
        while (percobaan < 3) {
        cout << "-------LOGINKAN DOLO-------" << endl;
        cout << "Masukkan Nama: ";
        cin >> input_nama;
        cout << "Masukkan Password: ";
        cin >> input_pw;
            if (input_nama == admin.username && input_pw == admin.password) {
            login_berhasil = true;
            cout << "Selamat datang! bos besarku, " << input_nama << endl;
            break;
        } else {
            percobaan++;
            cout << "Nama atau Password salah, sisa percobaan: "
                << 3 - percobaan << endl << endl;
        }
    }

    if (!login_berhasil) {
        cout << "telah gagal login 3 kali. cobalagi nanti ya.\n";
        return 0;
    }

    int pilihan;
    do {
        cout << "\n===== MENU UTAMA TOKO PERALATAN GAMING =====\n";
        cout << "1. Tambah Data Peralatan Gaming\n";
        cout << "2. Lihat Daftar Peralatan Gaming\n";
        cout << "3. Ubah Data Peralatan Gaming\n";
        cout << "4. Hapus Data Peralatan Gaming\n";
        cout << "5. Keluar\n";
        cout << "Pilih menu (1-5): ";
        cin >> pilihan;

        switch (pilihan) {

        case 1:
            tambahData(daftarGaming, totalData);
            break;

        case 2:
            lihatData(daftarGaming, totalData);
            break;

        case 3:
            ubahData(daftarGaming, totalData);
            break;

        case 4:
            totalData = hapusData(daftarGaming, totalData);
            break;

        case 5:
            cout << "Terima kasih telah berbelanja di gaming store terbaik lah\n";
            break;

        default:
            cout << "nomor tidak ada\n";
        }
    } while (pilihan != 5);
    return 0;
}